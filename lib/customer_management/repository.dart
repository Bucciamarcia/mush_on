...
class CustomerManagementRepository {
  ...
  Future<void> deleteCustomerGroup(String id) async {
    ...
    var col = await _db
        ...
        .get();
    for (var c in col.docs) {
      batch.delete(_db.doc("$bookingPath/${c.id}"));
    }
    try {
      await batch.commit();
    } catch (e, s) {
  ...
  Future<void> setCustomers(List<Customer> customers, String bookingId) async {
    logger.info("Called setCustomers");
    logger.debug("Customers in setCustomers: $customers");
    var batch = _db.batch();
    ...
    for (var c in snapshot.docs) {
      logger.debug("Deleting customer name: ${c.data()["name"]}");
      batch.delete(_db.doc("$path/${c.id}"));
    }

    for (Customer customer in customers) {
      var docRef = _db.doc("$path/${customer.id}");
      logger.debug("Setting customer: ${customer.name}");
