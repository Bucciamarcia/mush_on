          ...
          data: (account) {
            return ListView(
              children: [
                NameWidget(
                  ...
                  onNameChanged: (String newName) async {
                    try {
                      ...
                      logger.error(
                        "Couldn't change the name of the dog",
                        error: e,
                        stackTrace: s,
                      );
                      if (context.mounted) {
                ...
                const Divider(),
                ref
                    .watch(singleDogImageProvider(account, dog.id))
                    .when(
                      data: (data) {
                        return DogPhotoCard(
                          image: data,
                ...
                DogInfoWidget(
                  ...
                  onBirthdayChanged: (DateTime newBirthday) async {
                    try {
                      ...
                      logger.error(
                        "Couldn't change birthday",
                        error: e,
                        stackTrace: s,
                      );
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          errorSnackBar(context, "Couldn't change birthday"),
                        );
                      }
                    }
                  },
                ...
                DogTasksWidget(
                  tasksInMemory:
                      ref.watch(tasksProvider(null)).value ??
                      const TasksInMemory(),
                  dog: dog,
                  onTaskEdited: (t) async {
                    try {
                      ...
                      logger.error(
                        "Couldn't edit task in dog page.",
                        error: e,
                        stackTrace: s,
                      );
                      if (context.mounted) {
                  ...
                  onTaskDeleted: (t) async {
                    try {
                      ...
                      logger.error(
                        "Couldn't delete task in dog page.",
                        error: e,
                        stackTrace: s,
                      );
                      if (context.mounted) {
                ...
                DistanceWarningWidget(
                  ...
                  onWarningAdded: (w) {
                    var newW = List<DistanceWarning>.from(dog.distanceWarnings);
                    newW = [...newW, w];
                ...
                CustomFieldArea(
                  ...
                  onCustomFieldSaved: (r) {
                    try {
                      ...
                      updatedCf.removeWhere(
                        (t) => t.templateId == r.templateId,
                      );
                      updatedCf.add(r);
                  ...
                  onCustomFieldDeleted: (templateId) {
                    try {
                      ...
                      logger.error(
                        "Couldn't delete the custom field",
                        error: e,
                        stackTrace: s,
                      );
                      if (context.mounted) {
                ...
                DogRunDataWidget(
                  ref
                          .watch(dogTotalProvider(dogId: dog.id, cutoff: null))
                          .value ??
                      [],
                ),
                DogrunTableWidget(
